WITH CarAverages AS (SELECT c.name          AS car_name,
     ...
     LowPositionCars AS (SELECT ca.car_name,
                                ca.car_class,
                                ca.average_position,
                                ca.race_count
                         FROM CarAverages ca
                         WHERE ca.average_position > 3.0),
     ClassRaceCounts AS (SELECT c.class       AS car_class,
                                COUNT(r.race) AS total_race_count
                         FROM Cars c
                                  JOIN Results r ON c.name = r.car
                         GROUP BY c.class),
     ClassLowPositionCounts AS (SELECT lpc.car_class,
